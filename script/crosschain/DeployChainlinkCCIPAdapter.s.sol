...
contract DeployChainlinkCCIPAdapter is CrossChainDeployBase {
    function deploy(address router, address globalParams) public returns (address) {
        console2.log("Deploying ChainlinkCCIPAdapter...");
        ChainlinkCCIPAdapter adapter = new ChainlinkCCIPAdapter(router, IGlobalParams(globalParams));
        console2.log("ChainlinkCCIPAdapter deployed at:", address(adapter));
    ...
    function run() external {
        ...
        address adapter = deploy(config.destinationCcipRouter, globalParams);
        ...
        console2.log("CHAINLINK_CCIP_ADAPTER_ADDRESS", adapter);
    }
}
